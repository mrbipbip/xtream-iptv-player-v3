...
public class NSTPlayerVod {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case 3:
                    ...
                    return;
                case 4:
                    NSTPlayerVod.this.f49$.id(R.id.app_video_volume_box).gone();
                    NSTPlayerVod.this.f49$.id(R.id.app_video_brightness_box).gone();
                    NSTPlayerVod.this.f49$.id(R.id.app_video_fastForward_box).gone();
                    return;
                default:
                    return;
            }
    ...
    private final OnSeekBarChangeListener mSeekListener = new C21076();
    private long newPosition = -1;
    private final OnClickListener onClickListener = new C21021();
    public OnControlPanelVisibilityChangeListener onControlPanelVisibilityChangeListener = new C21065();
    private OnErrorListener onErrorListener = new C21032();
    private OnInfoListener onInfoListener = new C21054();
    private Runnable oncomplete = new C21043();
    ...
    class C21032 implements OnErrorListener {
        C21032() {
        }
    ...
    class C21043 implements Runnable {
        C21043() {
        }
    ...
    class C21054 implements OnInfoListener {
        C21054() {
        }
    ...
    class C21065 implements OnControlPanelVisibilityChangeListener {
        C21065() {
        }
    ...
    class C21076 implements OnSeekBarChangeListener {
        C21076() {
        }

        public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
            if (fromUser) {
                NSTPlayerVod.this.f49$.id(R.id.app_video_status).gone();
                ...
                NSTPlayerVod.this.f49$.id(R.id.app_video_currentTime).text(time);
            }
        }
    ...
    class C21098 implements IMediaPlayer$OnCompletionListener {
        C21098() {
        }
    ...
    class C21109 implements IMediaPlayer$OnErrorListener {
        C21109() {
        }
    ...
    public void setTitle(CharSequence title) {
        this.f49$.id(R.id.app_video_title).text(title);
    }

    public void doPauseResume() {
        if (this.status == this.STATUS_COMPLETED) {
            this.f49$.id(R.id.app_video_replay).gone();
    ...
    private void updatePausePlay() {
        if (this.videoView.isPlaying()) {
            this.f49$.id(R.id.exo_play).gone();
            this.f49$.id(R.id.exo_pause).visible();
            return;
        }
        this.f49$.id(R.id.exo_pause).gone();
        this.f49$.id(R.id.exo_play).visible();
    }
    ...
    public void show(int timeout) {
        if (!this.isShowing) {
            this.f49$.id(R.id.app_video_top_box).visible();
            this.f49$.id(R.id.app_video_bottom_box).visible();
            this.f49$.id(R.id.controls).visible();
            if (!this.isLive) {
                showBottomControl(true);
            }
            if (!this.fullScreenOnly) {
                this.f49$.id(R.id.app_video_fullscreen).visible();
            }
    ...
    public void showBottomControl(boolean show) {
        int i;
        int i2 = 0;
        Query id = this.f49$.id(R.id.app_video_currentTime);
        if (show) {
            i = 0;
        } else {
            i = 8;
        ...
        id.visibility(i);
        id = this.f49$.id(R.id.app_video_endTime);
        if (show) {
            i = 0;
        } else {
            i = 8;
        ...
        id.visibility(i);
        Query id2 = this.f49$.id(R.id.app_video_seekBar);
        if (!show) {
            i2 = 8;
        }
        id2.visibility(i2);
    ...
    public NSTPlayerVod(final Activity activity) {
        ...
        this.f49$ = new Query(activity);
        this.videoView = (IjkVideoView) activity.findViewById(R.id.video_view);
        this.videoView.setOnCompletionListener(new C21098());
        this.videoView.setOnErrorListener(new C21109());
        ...
        this.f49$.id(R.id.app_video_fullscreen).clicked(this.onClickListener);
        this.f49$.id(R.id.app_video_finish).clicked(this.onClickListener);
        this.f49$.id(R.id.app_video_replay_icon).clicked(this.onClickListener);
    ...
    private void statusChange(int newStatus) {
        ...
        if (!this.isLive && newStatus == this.STATUS_COMPLETED) {
            ...
            hideAll();
            this.f49$.id(R.id.app_video_replay).visible();
            ...
            hideAll();
            this.f49$.id(R.id.app_video_loading).visible();
    ...
    private void hideAll() {
        this.f49$.id(R.id.app_video_replay).gone();
        this.f49$.id(R.id.app_video_top_box).gone();
        this.f49$.id(R.id.app_video_bottom_box).gone();
        this.f49$.id(R.id.controls).gone();
        this.f49$.id(R.id.app_video_loading).gone();
        this.f49$.id(R.id.app_video_fullscreen).invisible();
        this.f49$.id(R.id.app_video_status).gone();
        ...
        if (this.videoView != null && !this.fullScreenOnly) {
            this.handler.post(new Runnable() {
                public void run() {
                    ...
                    if (portrait) {
                        NSTPlayerVod.this.f49$.id(R.id.app_video_box).height(NSTPlayerVod.this.initHeight, false);
                    } else {
                        NSTPlayerVod.this.f49$.id(R.id.app_video_box).height(Math.min(NSTPlayerVod.this.activity.getResources().getDisplayMetrics().heightPixels, NSTPlayerVod.this.activity.getResources().getDisplayMetrics().widthPixels), false);
                    }
    ...
    private void showStatus(String statusText) {
        this.f49$.id(R.id.app_video_status).visible();
        this.f49$.id(R.id.app_video_status_text).text(statusText);
    }
    ...
    private void onVolumeSlide(float percent) {
        ...
        this.f49$.id(R.id.app_video_volume_icon).image(i == 0 ? R.drawable.ic_volume_off_white_36dp : R.drawable.ic_volume_up_white_36dp);
        this.f49$.id(R.id.app_video_brightness_box).gone();
        this.f49$.id(R.id.app_video_volume_box).visible();
        this.f49$.id(R.id.app_video_volume_box).visible();
        this.f49$.id(R.id.app_video_volume).text(s).visible();
    }
    ...
    private void onProgressSlide(float percent) {
        ...
        if (showDelta != 0) {
            this.f49$.id(R.id.app_video_fastForward_box).visible();
            this.f49$.id(R.id.app_video_fastForward).text((showDelta > 0 ? "+" + showDelta : "" + showDelta) + "s");
            this.f49$.id(R.id.app_video_fastForward_target).text(generateTime(this.newPosition) + "/");
            this.f49$.id(R.id.app_video_fastForward_all).text(generateTime(duration));
        }
    }
    ...
    private void onBrightnessSlide(float percent) {
        ...
        this.f49$.id(R.id.app_video_brightness_box).visible();
        ...
        this.f49$.id(R.id.app_video_brightness).text(((int) (lpa.screenBrightness * 100.0f)) + "%");
    ...
    private long setProgress() {
        ...
        this.f49$.id(R.id.app_video_currentTime).text(generateTime(position));
        this.f49$.id(R.id.app_video_endTime).text(generateTime(this.duration));
        return position;
    }
    ...
    private void updateFullScreenButton() {
        if (getScreenOrientation() == 0) {
            this.f49$.id(R.id.app_video_fullscreen).image(R.drawable.ic_fullscreen_exit_white_36dp);
        } else {
            this.f49$.id(R.id.app_video_fullscreen).image(R.drawable.ic_fullscreen_white_24dp);
        }
    }
    ...
    public void setShowNavIcon(boolean show) {
        this.f49$.id(R.id.app_video_finish).visibility(show ? 0 : 8);
    }
