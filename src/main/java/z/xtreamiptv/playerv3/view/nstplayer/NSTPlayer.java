...
public class NSTPlayer {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case 3:
                    ...
                    return;
                case 4:
                    NSTPlayer.this.f46$.id(R.id.app_video_volume_box).gone();
                    NSTPlayer.this.f46$.id(R.id.app_video_brightness_box).gone();
                    NSTPlayer.this.f46$.id(R.id.app_video_fastForward_box).gone();
                    return;
                default:
                    return;
            }
    ...
    private final OnSeekBarChangeListener mSeekListener = new C20736();
    private long newPosition = -1;
    private final OnClickListener onClickListener = new C20681();
    public OnControlPanelVisibilityChangeListener onControlPanelVisibilityChangeListener = new C20725();
    private OnErrorListener onErrorListener = new C20692();
    private OnInfoListener onInfoListener = new C20714();
    private Runnable oncomplete = new C20703();
    ...
    class C20681 implements OnClickListener {
        C20681() {
        }
    ...
    class C20692 implements OnErrorListener {
        C20692() {
        }
    ...
    class C20714 implements OnInfoListener {
        C20714() {
        }
    ...
    class C20725 implements OnControlPanelVisibilityChangeListener {
        C20725() {
        }
    ...
    class C20736 implements OnSeekBarChangeListener {
        C20736() {
        }

        public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
            if (fromUser) {
                NSTPlayer.this.f46$.id(R.id.app_video_status).gone();
                ...
                NSTPlayer.this.f46$.id(R.id.app_video_currentTime).text(time);
            }
        }
    ...
    class C20758 implements IMediaPlayer$OnCompletionListener {
        C20758() {
        }
    ...
    class C20769 implements IMediaPlayer$OnErrorListener {
        C20769() {
        }
    ...
    public void setTitle(CharSequence title) {
        this.f46$.id(R.id.app_video_title).text(title);
    }

    public void doPauseResume() {
        if (this.status == this.STATUS_COMPLETED) {
            this.f46$.id(R.id.app_video_replay).gone();
    ...
    private void updatePausePlay() {
        if (this.videoView.isPlaying()) {
            this.f46$.id(R.id.exo_play).gone();
            this.f46$.id(R.id.exo_pause).visible();
            return;
        }
        this.f46$.id(R.id.exo_pause).gone();
        this.f46$.id(R.id.exo_play).visible();
    }

    public void show(int timeout) {
        if (!this.isShowing) {
            this.f46$.id(R.id.app_video_top_box).visible();
            this.f46$.id(R.id.controls).visible();
            this.f46$.id(R.id.ll_seekbar_time).visible();
            if (!this.isLive) {
                showBottomControl(true);
            }
            if (!this.fullScreenOnly) {
                this.f46$.id(R.id.app_video_fullscreen).visible();
            }
    ...
    public void showBottomControl(boolean show) {
        int i;
        int i2 = 0;
        Query id = this.f46$.id(R.id.app_video_currentTime);
        if (show) {
            i = 0;
        } else {
            i = 8;
        ...
        id.visibility(i);
        id = this.f46$.id(R.id.app_video_endTime);
        if (show) {
            i = 0;
        } else {
            i = 8;
        ...
        id.visibility(i);
        Query id2 = this.f46$.id(R.id.app_video_seekBar);
        if (!show) {
            i2 = 8;
        }
        id2.visibility(i2);
    ...
    public NSTPlayer(final Activity activity) {
        ...
        this.f46$ = new Query(activity);
        this.videoView = (IjkVideoView) activity.findViewById(R.id.video_view);
        this.videoView.setOnCompletionListener(new C20758());
        this.videoView.setOnErrorListener(new C20769());
        ...
        this.f46$.id(R.id.app_video_fullscreen).clicked(this.onClickListener);
        this.f46$.id(R.id.app_video_finish).clicked(this.onClickListener);
        this.f46$.id(R.id.app_video_replay_icon).clicked(this.onClickListener);
    ...
    private void statusChange(int newStatus) {
        ...
        if (!this.isLive && newStatus == this.STATUS_COMPLETED) {
            ...
            hideAll();
            this.f46$.id(R.id.app_video_loading).visible();
    ...
    private void hideAll() {
        this.f46$.id(R.id.app_video_replay).gone();
        this.f46$.id(R.id.app_video_top_box).gone();
        this.f46$.id(R.id.controls).gone();
        this.f46$.id(R.id.app_video_loading).gone();
        this.f46$.id(R.id.app_video_fullscreen).invisible();
        this.f46$.id(R.id.app_video_status).gone();
        this.f46$.id(R.id.ll_seekbar_time).gone();
        ...
        if (this.videoView != null && !this.fullScreenOnly) {
            this.handler.post(new Runnable() {
                public void run() {
                    ...
                    if (portrait) {
                        NSTPlayer.this.f46$.id(R.id.app_video_box).height(NSTPlayer.this.initHeight, false);
                    } else {
                        NSTPlayer.this.f46$.id(R.id.app_video_box).height(Math.min(NSTPlayer.this.activity.getResources().getDisplayMetrics().heightPixels, NSTPlayer.this.activity.getResources().getDisplayMetrics().widthPixels), false);
                    }
    ...
    private void showStatus(String statusText) {
        this.f46$.id(R.id.app_video_status).visible();
        this.f46$.id(R.id.app_video_status_text).text(statusText);
    }
    ...
    private void onVolumeSlide(float percent) {
        ...
        this.f46$.id(R.id.app_video_volume_icon).image(i == 0 ? R.drawable.ic_volume_off_white_36dp : R.drawable.ic_volume_up_white_36dp);
        this.f46$.id(R.id.app_video_brightness_box).gone();
        this.f46$.id(R.id.app_video_volume_box).visible();
        this.f46$.id(R.id.app_video_volume_box).visible();
        this.f46$.id(R.id.app_video_volume).text(s).visible();
    }
    ...
    private void onProgressSlide(float percent) {
        ...
        if (showDelta != 0) {
            this.f46$.id(R.id.app_video_fastForward_box).visible();
            this.f46$.id(R.id.app_video_fastForward).text((showDelta > 0 ? "+" + showDelta : "" + showDelta) + "s");
            this.f46$.id(R.id.app_video_fastForward_target).text(generateTime(this.newPosition) + "/");
            this.f46$.id(R.id.app_video_fastForward_all).text(generateTime(duration));
        }
    }
    ...
    private void onBrightnessSlide(float percent) {
        ...
        this.f46$.id(R.id.app_video_brightness_box).visible();
        ...
        this.f46$.id(R.id.app_video_brightness).text(((int) (lpa.screenBrightness * 100.0f)) + "%");
    ...
    private long setProgress() {
        ...
        this.f46$.id(R.id.app_video_currentTime).text(generateTime(position));
        this.f46$.id(R.id.app_video_endTime).text(generateTime(this.duration));
        return position;
    }
    ...
    private void updateFullScreenButton() {
        if (getScreenOrientation() == 0) {
            this.f46$.id(R.id.app_video_fullscreen).image(R.drawable.ic_fullscreen_exit_white_36dp);
        } else {
            this.f46$.id(R.id.app_video_fullscreen).image(R.drawable.ic_fullscreen_white_24dp);
        }
    }
    ...
    public void setShowNavIcon(boolean show) {
        this.f46$.id(R.id.app_video_finish).visibility(show ? 0 : 8);
    }
