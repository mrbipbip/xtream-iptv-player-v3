...
import z.xtreamiptv.playerv3.view.utility.epg.domain.EPGState;
import z.xtreamiptv.playerv3.view.utility.epg.misc.EPGDataImpl;
import z.xtreamiptv.playerv3.view.utility.epg.misc.EPGUtil;
...
public class EPG extends ViewGroup {
    ...
    class C21185 implements OnClickListener {
        C21185() {
        }
    ...
    private static class AsyncLoadProgramImage extends AsyncTask<EPGEvent, Void, Bitmap> {
        ...
        protected android.graphics.Bitmap doInBackground(z.xtreamiptv.playerv3.view.utility.epg.domain.EPGEvent... r8) {
            /* TODO */
            return null;
        }
    ...
    private void loadProgramDetails(EPGEvent epgEvent) {
        ...
        //this.loadProgramImage = new AsyncLoadProgramImage(this, epgEvent).execute(new EPGEvent[0]);  /* TODO */
        ...
        if (!(event.getKeyCode() == 4 || this.selectedEvent == null)) {
            if (event.getKeyCode() == 22) {
                ...
                if (this.selectedEvent.getChannel().getPreviousChannel() == null || this.selectedEvent.getChannel().getPreviousChannel().getChannelID() == this.epgDataLastChannelID.getChannelID()) {
                    ...
                    long eventMiddleTime = (Math.max(this.mTimeLowerBoundary, this.selectedEvent.getStart()) + Math.min(this.mTimeUpperBoundary, this.selectedEvent.getEnd())) / 2;
    ...
    private void EPGPlayPopUp(Context context, String selectedPlayer, int streamID, String num, String name, String epgChannelId, String epgChannelLogo) {
        ...
        closeLayout.setOnClickListener(new C21185());
    }
