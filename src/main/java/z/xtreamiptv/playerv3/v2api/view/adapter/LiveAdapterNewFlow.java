...
import z.xtreamiptv.playerv3.v2api.view.activity.LiveCategoryActivity;
...
//    }
    ...
    public void onBindViewHolder(final MyViewHolder holder, int listPosition) {
        ...
        int count = this.liveStreamsDatabaseHandler.getLiveStreamCountWithCat(data.getLiveStreamCategoryID());
        if (count == 0 || count == -1) {
        ...
        if (listPosition == 0 && data.getLiveStreamCategoryID().equals(AppConst.PASSWORD_UNSET)) {
            countAll = this.liveStreamsDatabaseHandler.getLiveStreamsCount();
    ...
    public void filter(final String text, final TextView tvNoRecordFound) {
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                ((Activity) LiveAdapterNewFlow.this.context).runOnUiThread(new C15831());
            }
        }).start();
    }
}
